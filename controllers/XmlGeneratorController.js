...
const generateSaftXml = async (req, res) => {
  try {
    ...
    // Build XML, creating the initial document and header section
    ...
    const company = doc.ele('d1:company')
      ...
      .ele('d1:vatCodeDetails') // Section 3.10
        ...
        .up()
      .up(); // Close vatCodeDetails

    // Only generate periods if year is valid
    if (year && !isNaN(Number(year))) {
      const periods = company.ele('d1:periods'); // Section 3.12
      for (let month = 1; month <= 12; month++) {
        const lastDay = new Date(year, month, 0).getDate();
        const monthName = new Date(year, month - 1).toLocaleString('da-DK', { month: 'long' });
        periods.ele('d1:period') // Section 3.13
    ...
    // Finalize the XML document and get the string

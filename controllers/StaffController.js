...
const createStaff = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Missing required fields: name, email, password, phone, manager_id, status, role",
        success: false,
      });
    }
...
const updateStaff = async (req, res) => {
  try {
    const { id, name, phone,  address, status, manager_id } = req.body;

    if (!id || !name || !phone || !manager_id || !status) {
      return res.status(400).json({

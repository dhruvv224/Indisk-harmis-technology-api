...
const getFood = async (req, res) => {
  try {
    const { manager_id, staff_id, is_manageable } = req.body;
    ...
    if (!is_manageable) {
      findBy.is_available = true
    }

    const foodList = await FoodItemSchema.find(findBy)
...
const foodModifier = async (req, res) => {
  try {
    const {
      id,
      discount,
    ...
    foodGet.discount = discount || [];
    foodGet.varient = varient || [];
    foodGet.modifier = modifier || [];
    foodGet.topup = topup || [];
    await foodGet.save();
    res.status(200).json({
      message: "Food item updated successfully",
...
const updateAvailabilityStatus = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message: "id and is_available fields required",
        success: false,
      });
    }

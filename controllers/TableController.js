  ...
  try {
    ...
    const enrichedTables = await Promise.all(
      tables.map(async (table) => {
        ...
        if (!activeOrder) {
          activeOrder = await OrderModel.findOne({
            table_no: table.table_no,
            status: "Prepared",
          });
        }
        ...
        const getOrderStatus = (activeOrder) => {
          if (!activeOrder) return null;
          return activeOrder?.status;
          // return activeOrder?.status === "Preparing" ? "Preparing" : "Prepared"
        };
        ...
        return {
          ...
          order_status: getOrderStatus(activeOrder),

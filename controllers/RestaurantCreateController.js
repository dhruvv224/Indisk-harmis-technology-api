...
const restaurantDetailsWithManagers = async (req, res) => {
  try {
    ...
    const managerList = await Manager.find({ restaurant_id: restaurant_id });
    ...
    const managerIds = managerList.map((manager) => manager.user_id);
    const managers = await UserAuth.find({ _id: { $in: managerIds } });
    if (!managers || managers.length === 0) {
      return res.status(400).json({
        message: "Manager user records not found!",
        success: false,
      });
    }
    ...
    const categories = await FoodCategoryModel.find({restaurant_id});

    const foods = await FoodItemModel.find({created_by: {$in:managerIds}}).populate({path:"category", select:"name"});

    const staff = await StaffModel.find({manager_id: {$in:managerIds}});
